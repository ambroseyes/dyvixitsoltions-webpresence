...
function RowBody({ index, title, body, mono = false }: { index: string; title: string; body: string; mono?: boolean }) {
  return (
    <>
      <span>
        <span className="rail-index">{index}</span>
        <span className="mt-2 block text-(length:--text-h4) font-semibold tracking-[-0.02em]">{title}</span>
        <span className={mono ? "mt-1.5 block font-mono text-(length:--text-sm) text-ink-muted" : "mt-1.5 block text-(length:--text-sm) text-ink-muted"}>
          {body}
        </span>
      </span>
      <ArrowRight
...
export function ClosingCTA({ lang }: { lang: Locale }) {
  ...
  return (
    <section aria-labelledby="cta-heading" className="grain relative overflow-hidden border-b border-line">
        ...
        <div className="grid items-end gap-12 py-(--spacing-section) lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
          <div>
            <p className="rail-label">
              <span className="mr-3 inline-block size-1.5 rotate-45 bg-primary align-middle" aria-hidden="true" />
              {t.nextStep}
            </p>
            <h2
              id="cta-heading"
