 ...
 * Insight articles.
 *
 * Content is a typed block array rather than MDX: it keeps the authoring
 * shape close to a CMS document (§38 — content should not be hard-coded into
 * components), avoids shipping an MDX runtime, and renders entirely on the
 * server.
 *
 * Authorship (§66): attributed to "D’Yvix Engineering" as an organisational
 * author. No fictional individual is invented. Replace with named author
 * profiles once real engineers agree to be credited.
 */
export type Block =
  | { type: "p"; text: string }
...
export const articles: Article[] = [
  {
    slug: "multi-wan-failover-that-actually-fails-over",
    ...
    relatedSolutions: ["infrastructure-cloud", "cybersecurity", "managed-services"],
    body: [
      {
        type: "p",
